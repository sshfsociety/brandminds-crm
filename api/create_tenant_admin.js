  ...
  try {
    await fetch(`${process.env.SUPABASE_URL}/rest/v1/audit_logs_v2`, {
      ...
      body: JSON.stringify({
        ...
        object_id: userId,
        details: { id: userId, email, tenant_id },
...
export default async function handler(req, res) {
  ...
  const { email, password, display_name, tenant_id } = req.body || {};
  ...
  try {
    ...
    // 2. Insert into users_meta
    const { error: insertErr } = await supabase.from('users_meta').insert({
      id: userId,
      tenant_id,
      role: 'tenant_admin',
